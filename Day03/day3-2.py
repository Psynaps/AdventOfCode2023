...
parts = [[] for i in range(len(lines))]
                ...
                if i > 0:
                    for k in range(
                        ...
                        if isSymbol(lines[i - 1][k]):
                            parts[i].append((numStart, numEnd, curNum))
                            curNum = -1
                            numStart = -1
                            numEnd = -1
                            break
...
for i, line in enumerate(lines):
    for j in range(len(line)):
        if line[j] == "*":
            # check how many parts are adjacent to this symbol
            adjacents = []
            if i > 0:
                for part in parts[i - 1]:
                    if j in range(part[0] - 1, part[1] + 2):
                        adjacents.append(part[2])
            if i < len(lines) - 1:
                for part in parts[i + 1]:
                    if j in range(part[0] - 1, part[1] + 2):
                        adjacents.append(part[2])
            for part in parts[i]:
                if part[0] == j + 1 or part[1] == j - 1:
                    adjacents.append(part[2])
            if len(adjacents) == 2:
